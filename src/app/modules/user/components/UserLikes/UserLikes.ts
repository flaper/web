...
export class UserLikes {
  ...
  getMixContent(ids, likes, fans) {
    let contents = {};
    let filter = {
      where: {id: {inq: ids['commentIds']}},
      limit: this.limit,
      fields: {id: true, subjectId: true, shortInline: true, subjectType: true}
    };
    this.comment.get(filter).subscribe(comments => {
      ...
      this.story.get(filter).subscribe(subStories => {
        ...
        let filter = {
          where: {id: {inq: ids['storyIds']}},
          limit: this.limit,
          fields: {id: true, title: true, slug: true, shortText: true}
        };
  ...
  findFanbase(likes, fans, contents) {
    ...
    likes.map(like => {
      ...
      if (data['fan'] && data['author'] && dataContent) list = {
        created: like['created'],
        type: like['subjectType'],
        fan: data['fan'],
        author: data['author'],
